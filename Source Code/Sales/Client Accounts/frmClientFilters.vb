...
Public Class frmClientFilters
    ...
    Public Sub LoadUserFilters()
        LoadXgridLookupEdit("select code, Description from tblclientfiltersettings", "tblclientfiltersettings", txtCoffeecupUser, s_type, Me)
    ...
    Public Sub ShowUnfilteredClient()
        ...
        com.CommandText = "select permissioncode from tblclientfilter where filtercode='" & code.Text & "'" : rst = com.ExecuteReader
        While rst.Read
    ...
    Public Sub ShowfilteredClients()
        If code.Text <> "" Then
            LoadXgrid("select permissioncode, (select authDescription from tbluserauthority where authCode= tblclientfilter.permissioncode) as 'User Permission' from tblclientfilter where filtercode='" & code.Text & "' order by (select authDescription from tbluserauthority where authCode= tblclientfilter.permissioncode)  asc", "tblclientfilter", Em_filtered, GridView2, Me)
    ...
    Private Sub SimpleButton1_Click(sender As Object, e As EventArgs) Handles cmdMoveRight.Click
        For I = 0 To GridView1.SelectedRowsCount - 1
            com.CommandText = "insert into tblclientfilter set permissioncode='" & GridView1.GetRowCellValue(GridView1.GetSelectedRows(I), "authCode").ToString & "', filtercode='" & code.Text & "'" : com.ExecuteNonQuery()
        Next
        ShowUnfilteredClient()
        ShowfilteredClients()
    End Sub
    ...
    Private Sub SimpleButton2_Click(sender As Object, e As EventArgs) Handles cmdMoveLeft.Click
        For I = 0 To GridView2.SelectedRowsCount - 1
            com.CommandText = "delete from tblclientfilter where permissioncode='" & GridView2.GetRowCellValue(GridView2.GetSelectedRows(I), "permissioncode").ToString & "' and filtercode='" & code.Text & "'" : com.ExecuteNonQuery()
        Next
        ShowUnfilteredClient()
        ShowfilteredClients()
    End Sub
